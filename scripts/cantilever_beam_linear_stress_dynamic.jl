...
applied_force = 0 # Total force applied along the right side in -y direction
nodes, elements, forces, fixed_dofs = create_cantilever_beam_linear(beam_length,
                                                                    beam_height, nx, ny,
                                                                    applied_force)
material = Material(1e5, 0.3, 6, 1)
u, K, f, σ = solve_fem_linear_stress_dynamic(nodes, elements, material, forces, fixed_dofs, n_timesteps, Δt, nx, ny, v_0)
